...
/**
 * Contract for medtrac Network
 */
class medtracNetContract extends Contract {
	constructor() {
		// Provide a custom name to refer to this smart contract
		super('org.medtrac-network.medtracnet')
		...
		global.manufacturerOrg = 'manufacturer.medtrac-network.com'
		global.distributorOrg = 'distributor.medtrac-network.com'
		global.transporterOrg = 'transporter.medtrac-network.com'
        global.retailerOrg = 'retailer.medtrac-network.com'
		global.consumerOrg = 'consumer.medtrac-network.com'
	}
	...
	validateInitiator(ctx, initiators) {
		...
		if (!matchOrg) {
			throw new Error(
				...
				initiatorID.issuer.organizationName.trim() +
				matchItem +
	...
	async instantiate(ctx) {
		console.log('medtrac Network Smart Contract Instantiated')
	}
	...
	async registerCompany(
		...
		let hierarchyKey = this.validateInitiator(ctx, [
			manufacturerOrg,
			distributorOrg,
			retailerOrg,
			transporterOrg,
		...
		const requestKey = ctx.stub.createCompositeKey(
			'org.medtrac-network.com.medtracnet.company',
			[companyCRN],
		)
	...
	async addDrug(ctx, drugName, serialNo, mfgDate, expDate, companyCRN) {
		...
		if (companyCRN) {
			...
			if (companyBuffer) {
				...
				if (companyObject.organisationRole === 'manufacturer') {
					...
					const drugKey = ctx.stub.createCompositeKey(
						'org.medtrac-network.com.medtracnet.drug',
						[drugName, serialNo],
					)
			...
			throw new Error('Company is not registered in the medtrac Network')
		}
	}
	...
	async createPO(ctx, buyerCRN, sellerCRN, drugName, quantity) {
		...
		if (
			(sellerObject.hierarchyKey == 1 && buyerObject.hierarchyKey === 2) ||
			(sellerObject.hierarchyKey == 2 && buyerObject.hierarchyKey == 3)
		) {
			const purchaseOrderKey = ctx.stub.createCompositeKey(
				'org.medtrac-network.com.medtracnet.purchaseOrder',
				[buyerCRN, drugName],
			)
			...
			throw new Error(
				"Purchase Order doesn't follow hierarchy. You can raise PO either with Manufacturer or Distributor",
			)
		}
	}
	...
	async createShipment(ctx, buyerCRN, drugName, listOfAssets, transporterCRN) {
		...
		if (assetAry.length == purchaseOrderObject.quantity) {
			for (let serialN in assetAry) {
				...
				let drugKey1 = ctx.stub.createCompositeKey(
					'org.medtrac-network.com.medtracnet.drug',
					[drugName, assetAry[serialN]],
				)
				let drugBuffer1 = await ctx.stub
					.getState(drugKey1)
					.catch((err) => console.log(err))
				...
				if (drugObject1) {
					...
					throw new Error(
						'The drug which you are trying to ship is not registered in the network',
					)
				}
			}
		} else {
			throw new Error(
				'Your shipment request will not be accepted as the count of drugs ' +
				assetAry.length +
				"shipped doesn't match with purchase Order " +
				purchaseOrderObject.quantity +
		...
		const shipmentKey = ctx.stub.createCompositeKey(
			'org.medtrac-network.com.medtracnet.shipment',
			[buyerCRN, drugName],
		)
	...
	async updateShipment(ctx, buyerCRN, drugName, transporterCRN) {
		...
		if (shipmentObject.transporter === transporterKey) {
			...
			for (let i in assets) {
				...
				if (
					buyerObject.organisationRole === 'distributor' ||
					buyerObject.organisationRole === 'retailer'
				) {
					drugObject.owner = ctx.stub.createCompositeKey(
						'org.medtrac-network.com.medtracnet.company',
						[buyerCRN],
					)
					let drugBuffer = Buffer.from(JSON.stringify(drugObject))
	...
	async retailDrug(ctx, drugName, serialNo, retailerCRN, customerAadhar) {
		...
		const retailerKey = ctx.stub.createCompositeKey(
			'org.medtrac-network.com.medtracnet.company',
			[retailerCRN],
		)
		...
		if (drugObject.owner == retailerKey) {
			...
			throw new Error(
				'Only owner of the property can sell the drug to consumer',
			)
		}
	}
...
module.exports = medtracNetContract
