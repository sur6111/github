...
createChannel() {
    ...
    if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
        set -x
        peer channel create -o orderer.medtrac-network.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/channel.tx >&log.txt
        res=$?
        set +x
    else
        set -x
        peer channel create -o orderer.medtrac-network.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/channel.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
        res=$?
        set +x
    fi
    cat log.txt
...
joinChannel() {
    ...
    for org in $ORGS; do
        for peer in 0 1; do
            ...
            echo "===================== peer${peer}.${org}.medtrac-network.com  joined channel '$CHANNEL_NAME' ===================== "
            sleep $DELAY
            echo
        done
    done
...
echo
echo "========= All GOOD, Hyperledger Fabric medtrac Network Is Now Up and Running! =========== "
echo
